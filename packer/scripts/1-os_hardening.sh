#!/bin/bash
### Hardening Script for RHEL Servers.
#RHEL 8 CIS Benchmark v 1.2
#Global CloudOps i.vcs.global.aws@SMOKESTACK.VERIFONE.com
...
cat > /etc/motd << 'EOF'
VERIFONE AUTHORIZED USE ONLY. All activity may be monitored and reported.
EOF
...
grep ':' /etc/passwd >> $AUDITDIR/reviewusrgrp_$TIME.log
grep ':' /etc/shadow >> $AUDITDIR/reviewusrgrp_$TIME.log
grep ':' /etc/group >> $AUDITDIR/reviewusrgrp_$TIME.log
...
rpm -q network-manager && nmcli radio all off
# =========================================================================
echo "Enabling faillock feature with authselect..."
rpm -q authselect && authselect select sssd >/dev/null
rpm -q authselect && authselect enable-feature with-faillock >/dev/null
rpm -q authselect && authselect apply-changes >/dev/null
# =========================================================================
echo "Set Crypto Policy to DEFAULT"
rpm -q crypto-policies && update-crypto-policies --set DEFAULT >/dev/null
# =========================================================================
...
echo "49.Installing AIDE..."
yum install aide -y
# =========================================================================
echo "50.Adding Bash timeout..."
cp /etc/bashrc $AUDITDIR/bashrc_$TIME.bak
egrep "TMOUT=600" /etc/bashrc || echo "TMOUT=600" >> /etc/bashrc
egrep "TMOUT=600" /etc/profile || echo "TMOUT=600" >> /etc/profile
#echo "TMOUT=600" >> /etc/bashrc
# =========================================================================
echo "51.Installing tcp_wrappers..."
yum install tcp_wrappers -y
# =========================================================================
echo "52.Changing permissions on bootloader config..."
chown root:root /boot/grub2/grub.cfg
chmod og-rwx /boot/grub2/grub.cfg
# =========================================================================
echo "53.Set permissions on all logfiles..."
find -L /var/log -type f -exec chmod g-wx,o-rwx {} +
# =========================================================================
echo "Please check $AUDITDIR"
# Cronjob for Advanced Intrusion Detection Environment (AIDE)
# For more information: https://access.redhat.com/documentation/es-es/red_hat_enterprise_linux/7/html/security_guide/sec-using-aide
#Install cronie
yum install cronie -y
# crontab -l > aidecheck
echo "0 5 * * * /usr/sbin/aide --check" >> aidecheck
crontab aidecheck
rm -f aidecheck
crontab -l
# #====================================================
...
#====================================================
# MOTD file contains OS information
# Remove execution permissions for /etc/motd file
chmod -x /etc/motd
#====================================================
...
#====================================================
# Remove telnet
rpm -q telnet && yum remove telnet -y
#====================================================
systemctl stop iptables && systemctl stop ip6tables && yum remove iptables* -y
#====================================================
rpm -q nftables && yum remove nftables -y
#====================================================
# The following command removes firewalld
rpm -q firewalld && yum remove firewalld -y
#====================================================
# Enable auditd
systemctl enable auditd
#====================================================
# Edit /etc/default/grub and add audit=1 to GRUB_CMDLINE_LINUX_DEFAULT
sed -i 's/\bGRUB_CMDLINE_LINUX_DEFAULT="\b/&audit=1 /' /etc/default/grub
# Update the grub2 configuration:
grub2-mkconfig -o /boot/grub2/grub.cfg

#====================================================
# https://www.stigviewer.com/stig/red_hat_enterprise_linux_6/2016-06-05/finding/V-38634
# Set the following parameter in /etc/audit/auditd.conf: max_log_file_action = keep_logs

sed -i "s/max_log_file_action =.*/max_log_file_action = keep_logs/I" /etc/audit/auditd.conf
# The auditd daemon can be configured to halt the system when the audit logs are full.
# Set the following parameters in /etc/audit/auditd.conf
sed -i "s/space_left_action =.*/space_left_action = email/I" /etc/audit/auditd.conf
sed -i "s/action_mail_acct =.*/action_mail_acct = root/I" /etc/audit/auditd.conf
sed -i "s/admin_space_left_action =.*/admin_space_left_action = halt/I" /etc/audit/auditd.conf
...
grep SELINUXTYPE=targeted /etc/selinux/config || echo "SELINUXTYPE=targeted" >> /etc/selinux/config
#====================================================
# Running SELinux in disabled mode is strongly discouraged
sed -i "s/SELINUX=.*/SELINUX=enforcing/I" /etc/selinux/config
#====================================================
sed -ri 's/(\\v|\\r|\\m|\\s)//g' /etc/motd
# Ensure permissions on /etc/motd are configured
chmod -t,u+r+w-x-s,g+r-w-x-s,o+r-w-x /etc/motd
#====================================================
yum erase 'ntp*' && yum install chrony
...
#====================================================
cat /etc/ntp.conf || touch /etc/ntp.conf
egrep -q "^\s*restrict(\s+-4)?\s+default(\s+\S+)*(\s*#.*)?\s*$" /etc/ntp.conf && sed -ri "s/^(\s*)restrict(\s+-4)?\s+default(\s+[^[:space:]#]+)*(\s+#.*)?\s*$/\1restrict\2 default kod nomodify notrap nopeer noquery\4/" /etc/ntp.conf || echo "restrict -4 default kod nomodify notrap nopeer noquery" >> /etc/ntp.conf
...
#====================================================
rpm -q iptables || yum -y remove iptables*
rpm -q firewalld || yum -y remove firewalld
#====================================================
rpm -q rsyslog || rpm -q syslog-ng || yum -y install rsyslog

echo "*.* action(type='omfwd' target='127.0.0.1' port='514' protocol='tcp' action.resumeRetryCount='100' queue.type='LinkedList' queue.size='1000')" | tee -a /etc/rsyslog.conf | tee -a /etc/rsyslog.d/*.conf
echo "*.* @@loghost.example.com" | tee -a /etc/rsyslog.conf | tee -a /etc/rsyslog.d/*.conf
echo "\$FileCreateMode 0640" | tee -a /etc/rsyslog.conf | tee -a /etc/rsyslog.d/*.conf
pkill -HUP rsyslogd
#====================================================
# Edit keys with the proper permissions
...
uname -p | grep -q 'x86_64' && egrep "^-a\s+(always,exit|exit,always)\s+-F\s+arch=b64\s+-S\s+sethostname\s+-S\s+setdomainname\s+-k\s+system-locale\s*$" /etc/audit/rules.d/audit.rules || echo "-a always,exit -F arch=b64 -S sethostname -S setdomainname -k system-locale 
-a always,exit -F arch=b32 -S sethostname -S setdomainname -k system-locale
-w /etc/issue -p wa -k system-locale
-w /etc/issue.net -p wa -k system-locale
-w /etc/hosts -p wa -k system-locale
...
#====================================================
egrep "^-w\s+/etc/selinux/\s+-p\s+wa\s+-k\s+MAC-policy\s*$" /etc/audit/rules.d/audit.rules || echo "-w /etc/selinux/ -p wa -k MAC-policy
-w /usr/share/selinux/ -p wa -k MAC-policy" >> /etc/audit/rules.d/audit.rules
egrep "^-w\s+/var/log/lastlog\s+-p\s+wa\s+-k\s+logins\s*$" /etc/audit/rules.d/audit.rules || echo "-w /var/log/lastlog -p wa -k logins 
-w /var/run/faillock/ -p wa -k logins" >> /etc/audit/rules.d/audit.rules

egrep "^-w\s+/var/run/utmp\s+-p\s+wa\s+-k\s+session\s*$" /etc/audit/rules.d/audit.rules || echo "-w /var/run/utmp -p wa -k session
-w /var/log/wtmp -p wa -k logins
-w /var/log/btmp -p wa -k logins" >> /etc/audit/rules.d/audit.rules

egrep "^-w\s+/etc/sudoers\s+-p\s+wa\s+-k\s+scope\s*$" /etc/audit/rules.d/audit.rules || echo "-w /etc/sudoers -p wa -k scope
...
#====================================================
egrep "Defaults use_pty" /etc/sudoers || echo "Defaults use_pty" >> /etc/sudoers
egrep "logfile" /etc/sudoers || echo 'Defaults logfile="/var/log/sudo.log"' >> /etc/sudoers
#====================================================
egrep -q "^\s*password\s+sufficient\s+pam_unix.so\s+" /etc/pam.d/password-auth && sed -ri '/^\s*password\s+sufficient\s+pam_unix.so\s+/ { /^\s*password\s+sufficient\s+pam_unix.so(\s+\S+)*(\s+sha512)(\s+.*)?$/! s/^(\s*password\s+sufficient\s+pam_unix.so\s+)(.*)$/\1sha512 \2/ }' /etc/pam.d/password-auth
egrep -q "^\s*password\s+sufficient\s+pam_unix.so\s+" /etc/pam.d/system-auth && sed -ri '/^\s*password\s+sufficient\s+pam_unix.so\s+/ { /^\s*password\s+sufficient\s+pam_unix.so(\s+\S+)*(\s+sha512)(\s+.*)?$/! s/^(\s*password\s+sufficient\s+pam_unix.so\s+)(.*)$/\1sha512 \2/ }' /etc/pam.d/system-auth

egrep -q "pam_pwhistory.so" /etc/pam.d/password-auth || echo "password required pam_pwhistory.so remember=5" >> /etc/pam.d/password-auth
egrep -q "pam_pwhistory.so" /etc/pam.d/system-auth || echo "password required pam_pwhistory.so remember=5" >> /etc/pam.d/system-auth
